...
/**
 * AdminBootstrap
 * Garante que o usuário Admin exista e esteja com a senha sincronizada com o
 * .env.
...
public class AdminBootstrap implements CommandLineRunner {
    ...
    public void run(String... args) {
        if (adminEmail == null || adminEmail.isBlank() || adminPassword == null || adminPassword.isBlank()) {
            System.out.println(
                    "⚠️ ADMIN_EMAIL e ADMIN_PASSWORD devem ser configurados via variáveis de ambiente. Pulando bootstrap de admin.");
            return;
        }
        ...
        userRepository.findByEmail(adminEmail).ifPresentOrElse(
                existingUser -> {
                    ...
                    // Só atualiza se o hash for diferente (evita update desnecessário, mas aqui
                    // forçamos por segurança)
                ...
                () -> {
                    ...
                    UserEntity newAdmin = new UserEntity("Admin Master", adminEmail,
                            passwordEncoder.encode(adminPassword), "ADMIN");

                    // Fallback se o Builder não estiver ativo no Entity
                    if (newAdmin.getName() == null) {
                        newAdmin = new UserEntity("Admin Master", adminEmail, passwordEncoder.encode(adminPassword),
                                "ADMIN");
                    }
