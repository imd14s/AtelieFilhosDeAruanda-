...
const AuthModal = ({ isOpen, onClose }) => {
  ...
  // onSuccess recebe { credential } — o ID Token JWT do Google
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      setLoading(true);
      setError("");
      try {
        // tokenResponse.access_token (fluxo implicit) ou tokenResponse.code (fluxo authorization_code)
        // Usamos o fluxo de token de acesso para buscar o perfil e depois enviar ao backend
        const res = await fetch("https://www.googleapis.com/oauth2/v3/userinfo", {
          headers: { Authorization: `Bearer ${tokenResponse.access_token}` },
        });
        const userInfo = await res.json();

        // Envia os dados para o backend criar/verificar a sessão
        ...
        <div className="p-8 md:p-10">
          ...
          {view === "LOGIN" && (
            <form onSubmit={handleLogin} className="space-y-5">
              ...
              {/* Botão Google — @react-oauth/google */}
              <button
                type="button"
                onClick={() => googleLogin()}
                disabled={loading}
              ...
              >
                ...
                {loading ? <Loader2 size={14} className="animate-spin" /> : "Entrar com Google"}
              </button>
