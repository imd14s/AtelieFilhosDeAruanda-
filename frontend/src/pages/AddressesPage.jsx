...
const AddressesPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingId) {
                await addressService.update(user.id, editingId, formData);
            } else {
                        ...
                        {addresses.map(addr => (
                            <div key={addr.id} className="bg-white rounded-md shadow-sm border border-gray-200 p-5 flex items-center justify-between hover:shadow-md transition">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button onClick={() => handleDelete(addr.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-md transition" title="Remover">
                                        <Trash2 size={16} />
                                    </button>
                                </div>
                            </div>
