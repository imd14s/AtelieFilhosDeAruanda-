                            ...
                            <div className="flex">
                                ...
                                <div className="w-24 bg-gradient-to-br from-blue-600 to-indigo-700 flex flex-col items-center justify-center p-2 text-white shrink-0">
                                    ...
                                    <div className="text-2xl font-bold leading-tight">
                                        {coupon.type === 'PERCENTAGE' || coupon.discountType === 'PERCENTAGE' ? `${coupon.value}%` : `R$${coupon.value}`}
                                    </div>
                                </div>
                                ...
                                <div className="flex-1 p-4 relative">
                                    <div className="flex justify-between items-start mb-2">
                                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider ${coupon.active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                                    ...
                                    <div className="space-y-1">
                                        <div className="flex items-center gap-2 text-xs text-gray-500">
                                            <Clock size={12} />
                                            <span>Expira em: {coupon.endDate ? new Date(coupon.endDate).toLocaleDateString('pt-BR') : 'Sem Expiração'}</span>
                                        </div>
                                        {((coupon.minPurchaseValue || 0) > 0 || (coupon.minPurchase || 0) > 0) && (
                                            <div className="text-[10px] text-gray-400">
                                                *Válido para compras acima de R$ {coupon.minPurchaseValue || coupon.minPurchase}
                                            </div>
                                        )}
                                    </div>
